...
def init_parents(recs):
 """ establish the 'parent' record for each rec. This
...
def init_children(recs):
 # get children of H1 and H2.  
 # skip H4 children for now
 parent = None
 nc = 0
 for irec,rec in enumerate(recs):
  ...
  if rec.H in ['3']:
   ...
   # skip children of children for now
   continue
  else:
   continue
...
def write_txt(recs,fileout):
 with codecs.open(fileout,"w","utf-8") as f:
  ...
  n2 = 0  # number of children in total
  for rec in recs:
   nc = len(rec.children)
   if nc == 0:
    continue
...
def write_md(recs,fileout):
 outarr = []
 outarr.append('## MW *compounds*')
 outarr = """## MW *compounds*
...
* is marked as H1 or H2 (MW's first or second line of headwords)
* and has one or more H3 *children*.
  * H3 headwords can also have H4 *children*, but this table ignores these

The columns of the table are:
* number of children
* the 'parent' headword.
  * The parent may be marked as a (VERB) -- 1247 of these.
    Clearly the children in such cases are not compounds, but the
    current (H3) markup of the children is the same as for samasas.
* the children (compounds) of the parent. In most cases, the
