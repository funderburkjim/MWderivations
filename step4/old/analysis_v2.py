...
def matchParent(firstpart,parentRec):
...
def analysis_wsfx(recs):
 ...
 for rec in recs:
  ...
  if matchParent(firstpart,parentRec) and (lastpart in Whitney_sfx.d) and (rec.status == 'TODO') and (parentRec.substantiveP()):
   # success
 ...
 for rec in recs:
  ...
  for ipart in xrange(1,nparts):
   ...
   if (not parentRec) or (not parentRec.substantiveP()):
    continue
   if matchParent(firstpart,parentRec) and (lastpart in drec) and (rec.status == 'TODO'):
    # success
    rec.analysis = "%s-%s" %(firstpart,lastpart)
    rec.status = 'DONE'
    rec.note = 'cpd1'
