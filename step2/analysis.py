...
def analysis_wsfx(recs):
 ...
 for rec in recs:
  ...
  if matchParent(firstpart,parentHkey) and (lastpart in Whitney_sfx.d) and (rec.status == 'TODO'):
   # success
...
def analysis_wsfx1(recs):
 ...
 for rec in recs:
  ...
  if (firstpart in drec) and (lastpart in Whitney_sfx.d):
   ...
   rec.status = 'DONE'
   ref = Whitney_sfx.d[lastpart]
   rec.note = 'wsfx1:%s:%s' %(lastpart, ref)
